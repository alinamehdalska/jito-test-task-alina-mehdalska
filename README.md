...
`tokens.json` is generated by script and **validated** by another: 184 leaf tokens,
...
tokens.json             W3C DTCG design tokens (184 tokens)
```
...
- **Screens are static specifications.** No prototype wiring and no motion
  implementation — motion is specified in `branding-strategy.md` §8 but not built.
- **Imagery is placeholder.** Hero and thumbnail areas use brand-tinted surfaces rather
  than food photography; the direction is specified on Stylescape board 3.
- **Icons are hand-drawn SVG**, not SF Symbols. Production would use the real symbol set;
  these exist so the screens read correctly without external assets.
- **Tab bar glyphs are placeholders** — deliberately generic squares rather than
  guessed-at icons.
- **The component library is not published.** Publishing requires a paid Figma plan;
  the components and variables are local to the file and fully usable there.
- **No production code.** This is a design deliverable; `tokens.json` is the handoff
  boundary.
