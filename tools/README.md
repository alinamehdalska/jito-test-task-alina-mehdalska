...
**The originals are not wrapped.** Putting bezels around the live frames on `04 · Screens`
would nest every prototype destination inside a parent frame and break the 48 reactions
wired the same day — and Figma's presentation view already draws its own device chrome, so
a bezel there would double up. `07 · Presentation` holds clones instead, generated by this
script rather than maintained by hand: regeneration, not vigilance, is what stops them
drifting. Same reasoning as the 1:1 stylescape clones.

**Clones carry reactions.** Every cloned screen arrives with the original's interactions
still pointing at frames on another page — dangling cross-page destinations. They are
stripped on build and the board audits at **0 reactions**.

### Geometry

393 × 852 is the iPhone 15 Pro's logical size, which is exactly what every frame is drawn
at, so nothing is scaled — no rewritten font sizes, no detached styles. Bezel 16, outer
425 × 884 at radius 71 (= 55 + 16, concentric with the 55 screen radius). The screen well
is cut on the clone itself: a frame's `clipsContent` respects its corner radius, so one
property rounds the screen. The **Dynamic Island already lives in each screen's own Apple
status bar**, so the bezel adds enclosure and side buttons only — drawing a notch would put
two islands on every screen. Enclosure is `neutral/900`, the file's warm black and the same
tint every shadow is built on; buttons are `neutral/700`.

### Spacing is structural, not eyeballed

One auto-layout board, every value on the documented scale: board padding **96**, row gap
**96**, label-to-rail **32**, device gap **48**, caption gap **16**. The three rails share a
left edge (x = 96) because they are children of one vertical stack, not because they were
nudged into line. Rows group by what the screens do — the two user stories, then the states
that serve both — rather than by frame number.

### Two traps

**Text styles carry type but not colour.** The 16 labels created here took `Subhead
Emphasized` / `Title 2` / `Large Title` correctly and still defaulted to an *unbound black*,
which failed the file's own "solid fills are 100% variable-bound" rule. Only the fill audit
caught it. Board text is now bound to `text.primary` / `text.secondary`.

**A depth-limited ancestor walk is not a containment test.** The filter meant to exclude
text *inside* devices walked up only 8 parents, so nine macro labels buried deeper in the
cloned dashboards were rebound too. They happened to already be `text/secondary`, so it was
a no-op — verified, not assumed, by diffing all 11 clones against their originals across
323 text nodes: **0 differences** in characters or bound token.
